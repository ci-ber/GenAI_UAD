...
- files in directory
"""
from transforms.preprocessing import AddChannelIfNeeded, AssertChannelFirst, ReadImage, To01, Pad
...
class DefaultDataset(Dataset):
    ...
    def get_image_transform(self):
        ...
        default_t = transforms.Compose([ReadImage(), #To01(),
                                        Pad(type='center'), AddChannelIfNeeded(),
                                        AssertChannelFirst(), transforms.Resize(self.target_size)])
        return default_t

    def get_image_transform_test(self):
        default_t = transforms.Compose([ReadImage(), To01(),  Pad(type='center'), AddChannelIfNeeded(),
                                        AssertChannelFirst(), transforms.Resize(self.target_size)])
        return default_t
    ...
    def get_label(self, idx):
        if self.label_files is not None:
            ...
            return self.seg_t(self.files[idx])


    def __getitem__(self, idx):
        try:
            return self.im_t(self.files[idx]), self.get_label(idx), self.files[idx]
        except:
            print(f'Error at: {self.files[idx]}')
            return self.im_t(self.files[0]), self.get_label(0), self.files[0]
            # exit()
