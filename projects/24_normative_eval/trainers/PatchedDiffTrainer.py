    ...
    def train(self, model_state=None, opt_state=None, start_epoch=0):
        ...
        for epoch in range(self.training_params['nr_epochs']):
            ...
            if self.early_stop is True:
                ...
                break
            start_time = time()
            batch_loss, batch_loss_pl, batch_loss_mae, count_images = 0.0, 0.0, 0.0, 0
            ...
            for data in self.train_ds:
                ...
                reconstructed_images, f_result = self.model(transformed_images, is_train=True)
                loss_mae = f_result['loss']
                # Reconstruction Loss
                ...
                batch_loss_mae += loss_mae.item() * images.size(0)
            ...
            epoch_loss_mae = batch_loss_mae / count_images if count_images > 0 else batch_loss_mae
            epoch_losses.append(epoch_loss)
            epoch_losses_pl.append(epoch_loss_pl)
            epoch_losses_mae.append(epoch_loss_mae)
            ...
            wandb.log({"Train/Loss_MAE_": epoch_loss_mae, '_step_': epoch})
            ...
            masked = f_result['mask'].detach()
            # masked = masked.unsqueeze(-1).repeat(1, 1, 256)
            # masked = self.model.unpatchify(masked)
            ...
            elements = [img, masked, rec, np.abs(rec - img)]
            v_maxs = [1, 1, 1, 0.5]
    ...
    def test(self, model_weights, test_data, task='Val', opt_weights=None, epoch=0):
        ...
        with torch.no_grad():
            for data in test_data:
                ...
                # Forward pass
                x_, x_rec = self.test_model(x, is_train=True)
                loss_mae = x_rec['loss']
                ...
                metrics[task + '_loss_mae'] += loss_mae.item() * x.size(0)


        img = x.detach().cpu()[0].numpy()
        rec = x_.detach().cpu()[0].numpy()
        masked = x_rec['mask'].detach()
        # masked = masked.unsqueeze(-1).repeat(1, 1, 256)
        # masked = self.model.unpatchify(masked)
        masked = masked[0].cpu().numpy()
        # masked += img
        ...
        epoch_val_loss = metrics[task + '_loss_mae'] / test_total
        if task == 'Val':
