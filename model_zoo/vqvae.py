...
from net_utils.layers.vector_quantizer import EMAQuantizer, VectorQuantizer
...
class VQVAE(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        return reconstruction, {'quantization_losses': quantization_losses}

    def get_anomaly(self, images):
        reconstruction, _ = self.forward(images)
        x_rec_ = reconstruction.cpu().detach().numpy()
        x_ = images.cpu().detach().numpy()
        anomaly_maps = np.abs(x_ - x_rec_)
        anomaly_score = np.mean(anomaly_maps, axis=(1, 2, 3), keepdims=True)
        return anomaly_maps, anomaly_score, {'x_rec': reconstruction}
