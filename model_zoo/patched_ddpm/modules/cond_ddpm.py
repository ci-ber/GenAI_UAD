...
# constants
"""
Code from: https://github.com/FinnBehrendt/patched-Diffusion-Models-UAD/tree/main
"""
...
class GaussianDiffusion(nn.Module):
    ...
    def model_predictions(self, x, t, cond, cond_scale, clip_x_start=False):
        if self.use_spatial_transformer:
            if x.shape[1] == 2:  # if channel conditioning is used, we indicate the patch by ones
                x[:, 1][x[:, 1] != -1] = 1
            model_output = self.model.forward_with_cond_scale(x, t, cond=cond, context=cond.unsqueeze(1),
                                                              cond_scale=cond_scale)  # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond
            if x.shape[1] == 2:
                x = x[:, 0].unsqueeze(1)
        else:
            model_output = self.model.forward_with_cond_scale(x, t, cond=cond,
                                                              cond_scale=cond_scale)  # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond
    ...
    def p_sample_loop(self, shape, cond=None, cond_scale=1., box=None, start_t=0, noise=None, x_start=None):
        ...
        if box is not None:
            ...
            for i in range(img.shape[0]):
                img_patch[i, :, box[i, 1]:box[i, 3], box[i, 0]:box[i, 2]] = img[i, :, box[i, 1]:box[i, 3],
                                                                            box[i, 0]:box[i, 2]]
                img = img_patch
    ...
    def ddim_sample(self, shape, clip_denoised=True, cond=None, cond_scale=1., x_start=None, start_t=0, noise=None):
        ...
        if start_t != 0:
            img = self.q_sample(x_start=x_start, t=torch.tensor([start_t], device=device), noise=noise)[:, 0].unsqueeze(
                1)
        else:
    ...
    def sample(self, batch_size=1, cond=None, cond_scale=1., box=None, x_start=None, start_t=0, noise=None):
        ...
        if box is not None:
            ...
            return sample_fn((batch_size, channels, image_size_h, image_size_w), x_start, cond=cond,
                             cond_scale=cond_scale, box=box, start_t=start_t, noise=noise)
        else:
            return sample_fn((batch_size, channels, image_size_h, image_size_w), cond=cond, cond_scale=cond_scale,
                             start_t=start_t, noise=noise, x_start=x_start)
    ...
    def p_losses(self, x_start, t, cond=None, noise=None, box=None, scale_patch=1, onlybox=False, mask=None):
        ...
        x = self.q_sample(x_start=x_start, t=t,
                          noise=noise)  # generate a noisy image from the start image at timestep t
        ...
        model_out = self.model(x, t,
                               cond=None)  # predict the noise that has been added to x_start or directly predict x_start from the noisy x, conditioned by the timestep t, cond

        if self.objective == 'pred_noise':  # predict the noise that has been added to x_start
            if exists(box):  # if box is not None, we only want to denoise the box region
                noise_patch = torch.zeros_like(noise)
                for i in range(noise.shape[0]):
                    noise_patch[i, :, box[i, 1]:box[i, 3], box[i, 0]:box[i, 2]] = noise[i, :, box[i, 1]:box[i, 3],
                                                                                  box[i, 0]:box[i, 2]]
                target = noise_patch
            else:
                target = noise
        ...
        if self.inpaint:
            ...
            for i in range(out.shape[0]):
                out[i, :, box[i, 1]:box[i, 3], box[i, 0]:box[i, 2]] = model_out[i, :, box[i, 1]:box[i, 3],
                                                                      box[i, 0]:box[i, 2]]
            model_out = out
        ...
        if self.objective == 'pred_noise':
            return loss.mean(), unnormalize_to_zero_to_one(
                x - extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape) * model_out)
        else:
            return loss.mean(), unnormalize_to_zero_to_one(model_out)

    def forward(self, img, t=None, *args, **kwargs):
        b, c, h, w, device, img_size, = *img.shape, img.device, self.image_size
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long() if t is None else (
                    torch.ones([b], device=device) * t).long()
