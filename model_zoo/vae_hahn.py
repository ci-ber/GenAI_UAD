...
import math
# from https://github.com/MIC-DKFZ/vae-anomaly-experiments/
# Apache Licence 2.0
#
# D. Zimmerer, F. Isensee, J. Petersen, S. Kohl, and K. Maier-Hein. Unsupervised anomaly localization using variational
# auto-encoders. In International Conference on Medical Image Computing and Computer-Assisted Intervention,
# pages 289–297. Springer, 2019.
...
class VAE(torch.nn.Module):
    ...
    def forward(self, inpt, sample=True, **kwargs):
        ...
        return x_rec, {'z_mu': mu, 'z_logvar': std}
    ...
    def get_anomaly(self, inpt, **kwargs):
        x_rec, _ = self.forward(inpt, **kwargs)
...
class VAELoss:
    # from https://github.com/MIC-DKFZ/vae-anomaly-experiments/
    def __init__(self, rec_log_std=0, sum_samplewise=True):
        super(VAELoss, self).__init__()
        self.rec_log_std = rec_log_std
        self.sum_samplewise = sum_samplewise

    def __call__(self, x_recon, x, z):
        mu = z['z_mu']
        logstd = z['z_logvar']

        rec_std = math.exp(self.rec_log_std)
        rec_var = rec_std ** 2

        x_dist = dist.Normal(x, rec_std)
        log_p_x_z = x_dist.log_prob(x_recon)
        if self.sum_samplewise:
...
class ceVAELoss:
    # from https://github.com/MIC-DKFZ/vae-anomaly-experiments/
    def __init__(self, rec_log_std=0, sum_samplewise=True):
        super(ceVAELoss, self).__init__()
        self.rec_log_std = rec_log_std
        self.sum_samplewise = sum_samplewise
        self.mse_loss = torch.nn.MSELoss()

    def __call__(self, x_recon, x_recon_ce, x, z):
        mu = z['z_mu']
        logstd = z['z_logvar']
        ...
        loss_rec = self.mse_loss(x, x_recon)
        los_rec_ce = self.mse_loss(x, x_recon_ce)

        # return torch.mean(kl_div + loss_rec + los_rec_ce)

        if self.sum_samplewise:
            kl_div = torch.sum(kl_div, dim=(1, 2, 3))

        loss = torch.mean(kl_div + loss_rec + los_rec_ce)
